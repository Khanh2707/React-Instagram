...
export function changeIconWhenActive() {
  ...
  const classNavigation__itemLogoNo_active = cx(
    "navigation__item-logo--no_active"
  );
  ...
  navigation__items.forEach(function (item) {
    if (
      !(
        item.classList.contains(classSearch_input_in_page_search) ||
        item.classList.contains(classNavigation__itemProfile)
      )
    ) {
      if (item.classList.contains(classActive)) {
        noActiveElement = item.querySelector(
          "." + classNavigation__itemLogoNo_active
        );
