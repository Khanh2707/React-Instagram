...
function Search({ searchIsActive }) {
  ...
  function reRenderSearchHistory() {
    http.get(`api/search_history/${idUser}`).then((res) => {
  ...
  useEffect(() => {
    ...
    http
      .get(`api/users/${encodeURIComponent(debounced)}/${idUser}`)
      .then((res) => {
  ...
  function handleSearchItemClickRemove(value) {
    http.del(`api/search_history/${idUser}/${value}`).then(() => {
      reRenderSearchHistory();
    });
  }

  function handleSearchItemClickRemoveAll() {
    http.del(`api/search_history`).then(() => {
      reRenderSearchHistory();
    });
  }
  ...
  function handleClickInputSearch() {
    const search_input_in_page_searchPC = document.querySelector(
      "." + cx("search_input_in_page_search-pc")
    );
    const search_input_in_page_searchPC_svg = document.querySelector(
      "." + cx("search_input_in_page_search-pc") + " svg"
    );
    const search_input_in_page_searchPC_input = document.querySelector(
      "." + cx("search_input_in_page_search-pc") + " input"
    );
    const search_input_in_page_searchPC_i = document.querySelector(
      "." + cx("search_input_in_page_search-pc") + " i"
    );
    ...
    search_input_in_page_searchPC.addEventListener("click", function (event) {
      ...
      if (
        event.target.closest("svg.fa-circle-xmark") !== null ||
    ...
    function removeAllCharacter() {
      ...
      pageSearch__recently.style.borderTop =
        "1px solid var(--border-navigation_bar)";
  ...
  return (
    <div
      className={cx("page-search", {
        animationAppearPageSearch: searchIsActive,
      })}
      style={{ display: isFirstActive ? "block" : "" }}
    >
      ...
      <div className={cx("page-search__input")}>
        <div
          className={cx(
            "navigation__item",
            "search_input_in_page_search",
            "search_input_in_page_search-pc"
          )}
          data-level='3'
        >
          <input
          ...
          <svg
            aria-label='Tìm kiếm'
            className={cx("x1lliihq x1n2onr6 x1cp0k07")}
            fill='currentColor'
            height='16'
            role='img'
            viewBox='0 0 24 24'
      ...
      <div
        className={cx("page-search__recently")}
        ref={pageSearch__recentlyRef}
      >
        <div
          className={cx("page-search__recently__header")}
          ref={pageSearch__recently__header}
        >
          <div className={cx("page-search__recently__header__title")}>
            <span>Gần đây</span>
          </div>
          <div
            className={cx("page-search__recently__header__delete_all")}
            onClick={handleSearchItemClickRemoveAll}
          >
              ...
              ? searchResult.map((result) => {
                  return (
                    <li key={result.idUser}>
                      <div
                        className={cx(
                          "page-search__recently__body__ul__li-infor"
                        )}
                        onClick={() => handleSearchItemClick(result.idUser)}
                      >
                        <div
                          className={cx(
                            "page-search__recently__body__ul__li-img"
                          )}
                        >
                          <img
                            src={
                        ...
                        <div
                          className={cx(
                            "page-search__recently__body__ul__li-short_infor"
                          )}
                        >
                          <div
                            className={cx(
                              "page-search__recently__body__ul__li-short_info__name_account"
                            )}
                          >
                          ...
                          <div
                            className={cx(
                              "page-search__recently__body__ul__li-short_info__name_real_and_amount_follower"
                            )}
                          >
                            <span
                              className={cx(
                                "page-search__recently__body__ul__li-short_info__name_real"
                              )}
                            >
                              {result.name}
                            </span>
                  ...
                  return (
                    <li key={result.user2.idUser}>
                      <div
                        ...
                        onClick={() =>
                          handleSearchItemClick(result.user2.idUser)
                        }
                      >
                        <div
                          className={cx(
                      ...
                      <div
                        className={cx(
                          "page-search__recently__body__ul__li-cancel"
                        )}
                        onClick={() =>
                          handleSearchItemClickRemove(result.user2.idUser)
                        }
                      >
                        <svg
                          aria-label='Đóng'
