...
function Message() {
  const { idUser, quantityMessageNotCheck, setQuantityMessageNotCheck } =
  ...
  useEffect(() => {
    if (params !== "inbox") getUserById(params);
  }, [params]);
  ...
  const [
    recentUserMessagesWithOtherUsers,
    setRecentUserMessagesWithOtherUsers,
  ] = useState([]);

  const getRecentUserMessagesWithOtherUsers = () => {
    http
      .get(`api/user_message/all_latest_message_by_user_1/${idUser}`)
      .then((res) => {
        console.log(res);
  ...
  useEffect(() => {
    if (idUser !== "") getRecentUserMessagesWithOtherUsers();
  }, [idUser]);
  ...
  const getMessagesWithOtherUser = (idUser1, idUser2) => {
    http.get(`api/user_message/${idUser1}/${idUser2}`).then((res) => {
      console.log(res);
  ...
  const recallMessage = (idUserMessage) => {
    http.del(`api/user_message/${idUserMessage}`).then((res) => {
      getRecentUserMessagesWithOtherUsers();
      getMessagesWithOtherUser(idUser, userTargetMessage.idUser);
      if (stompClient) {
        var chatMessage = {
  ...
  return (
    <div className={cx("main", "page-message")}>
          ...
          <ul>
            {recentUserMessagesWithOtherUsers
              ...
              .map((res) => {
                ...
                return (
                  <li
                    key={res.idUserMessage}
                    className={cx(
                      "list_inbox__content__ul__li",
                      isActive ? "active" : ""
                    )}
                    onClick={() =>
                  ...
                  >
                    <div className={cx("list_inbox__content__ul__li-img")}>
                      <img
                        src={
                          idUser === res.user1.idUser
                            ? res.user2.avatar === null
                              ? defaultAvatar
                              : res.user2.avatar
                            : res.user1.avatar === null
                            ? defaultAvatar
                            : res.user1.avatar
                        }
                        alt=''
                      ...
                      <div
                        className={cx(
                          "list_inbox__content__ul__li-icon_activing"
                        )}
                      ></div>
                    </div>
                    <div
                      className={cx(
                        "list_inbox__content__ul__li-name_user_and_active_state_or_new_inbox"
                      )}
                    >
                      <div
                        className={cx("list_inbox__content__ul__li-name_user")}
                      >
                        <span>
                          {idUser === res.user1.idUser
                            ? res.user2.name
                      ...
                      <div
                        className={cx(
                          "list_inbox__content__ul__li-inbox_and_time"
                        )}
                      >
                        <span
                          className={cx(
                            idUser !== res.user1.idUser && res.isCheck === false
                              ? "list_inbox__content__ul__li-inbox_has_been_no_read"
                              : "list_inbox__content__ul__li-inbox_has_been_read"
                          )}
                        >
                          {res.message}
                        </span>
                    ...
                    <div
                      className={cx(
                        "list_inbox__content__ul__li-icon_turn_off_notification_message"
                      )}
                    >
                      <svg
                        aria-label='Đã tắt tiếng'
                        className={cx("x1lliihq x1n2onr6 x1roi4f4")}
                        fill='currentColor'
                        height='15'
                        role='img'
                        viewBox='0 0 24 24'
                    ...
                    <div
                      className={cx(
                        "list_inbox__content__ul__li-icon_notification_new_message",
                        idUser !== res.user1.idUser && res.isCheck === false
                          ? "active"
                          : ""
                      )}
      ...
      <div className={cx("not_select_inbox")} ref={not_select_inboxRef}>
        ...
        <span className={cx("not_select_inbox__describe")}>
      ...
      <div className={cx("detail_inbox")} ref={detail_inboxRef}>
        <div className={cx("detail_inbox__header")}>
          <div className={cx("detail_inbox__header__user")}>
            <div
              className={cx("detail_inbox__header__user-img")}
              onClick={() =>
                (window.location.href = `/${userTargetMessage.idUser}`)
              }
            >
              <img
                src={
                  userTargetMessage.avatar === null
                    ? defaultAvatar
                    : userTargetMessage.avatar
                }
                alt=''
              />
              <div
                className={cx("list_inbox__content__ul__li-icon_activing")}
              ></div>
            </div>
            <div
              className={cx(
                "detail_inbox__header__user-name_user_and_active_state"
              )}
            >
              <div
                className={cx("detail_inbox__header__user-name_user")}
                onClick={() =>
                  (window.location.href = `/${userTargetMessage.idUser}`)
                }
              >
              ...
              .map((res) => {
                if (res.user1.idUser === idUser) {
                  return (
                    <div
                      key={res.idUserMessage}
                      className={cx("detail_inbox__body__message__item")}
                      onMouseEnter={() =>
                        isOptionsMessageCLicked === null &&
                        setHoveredMessage(res.idUserMessage)
                      }
                    >
                      <div
                        className={cx(
                          "detail_inbox__body__message__item__message",
                          "detail_inbox__body__message__item__message-me"
                        )}
                      >
                        <span>{res.message}</span>
                        <div
                          className={cx(
                            "detail_inbox__body__message__item__options_message",
                            hoveredMessage === res.idUserMessage ? "active" : ""
                          )}
                          onClick={() => {
                        ...
                        >
                          ...
                          <div
                            className={cx(
                              "detail_inbox__body__message__item__options_message__container",
                              activeOptionsMessage === res.idUserMessage
                                ? "active"
                                : ""
                            )}
                            onClick={(e) => {
                          ...
                          >
                            <div
                              className={cx(
                                "detail_inbox__body__message__item__options_message__item",
                                "detail_inbox__body__message__item__options_message__item-time"
                              )}
                            >
                              <span>
                                {new Date(res.dateTimeMessage).toLocaleString()}
                              </span>
                            </div>
                            <div
                              className={cx(
                                "detail_inbox__body__message__item__options_message__item",
                                "detail_inbox__body__message__item__options_message__item-recall"
                              )}
                              onClick={() => {
                                setIsOptionsMessageClicked(null);
                  ...
                  return (
                    ...
                    >
                      <div
                        className={cx(
                          "detail_inbox__body__message__item__message",
                          "detail_inbox__body__message__item__message-you"
                        )}
                      >
                        <span>{res.message}</span>
                      </div>
        ...
        <div className={cx("detail_inbox__footer")}>
          <div className={cx("detail_inbox__footer__input_inbox")}>
            ...
            <div
              className={cx(
                "detail_inbox__footer__input_inbox__icon_option__group_right"
              )}
            >
            ...
            <div
              className={cx("detail_inbox__footer__input_inbox__button_submit")}
      ...
      <div className={cx("information_detail_inbox")}>
        ...
        <div className={cx("information_detail_inbox__body")}>
          <div
            className={cx(
              "information_detail_inbox__body__turn_off_ring_inbox"
            )}
          >
            <span>Tắt thông báo về tin nhắn</span>
            <label
              className={cx(
                "information_detail_inbox__body__turn_off_ring_inbox__switch_button"
              )}
            >
              <input type='checkbox' />
              <span
                className={cx(
                  "information_detail_inbox__body__turn_off_ring_inbox__switch_button-slider"
                )}
              ></span>
            </label>
          </div>
          ...
          <div
            className={cx("information_detail_inbox__body__list_member")}
          ></div>
          <div
            className={cx(
              "information_detail_inbox__body__list_button_negative"
            )}
          >
            <div
              className={cx(
                "information_detail_inbox__body__list_button_negative-button"
              )}
            >
              <span>Chặn</span>
            </div>
