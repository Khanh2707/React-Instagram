...
function Home() {
  ...
  const [width, setWidth] = useState(
    window.innerWidth <= 1263 ? 316 - (1263 - window.innerWidth) / 10 : 316
  );
  const [widthContainerPost, setWidthContainerPost] = useState(
    window.innerWidth <= 1263
      ? 960 - ((1263 - window.innerWidth) / 10) * 3
      : 960
  );
  ...
  const [allPostRandomExcludingMyUser, setAllPostRandomExcludingMyUser] =
    useState([]);

  const getAllPostRandomExcludingMyUser = () => {
    http.get(`api/posts/random_excluding_user/${idUser}`).then((res) => {
  ...
  useEffect(() => {
    if (idUser !== "") getAllPostRandomExcludingMyUser();
  }, [idUser]);
  ...
  return (
    <div className={cx("main", "page-home")}>
      <div
        className={cx("container_post")}
        style={{ width: `${widthContainerPost}px` }}
      >
