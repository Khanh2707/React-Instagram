...
function DetailPostPage() {
  ...
  const { idUser, sendPostNotification } = useContext(AppContext);
  ...
  const [listAllUserCommentPostByPost, setListAllUserCommentPostByPost] =
    useState([]);

  const getAllUserCommentPostByPost = () => {
    http.get(`api/user_comment_post/by_post/${params}`).then((res) => {
      console.log(res);
  ...
  const handleOpenOptionsComment = (idUser, idCommentPost) => {
    openModalTwo(
      <OptionsComment
        ...
        idCommentPost={idCommentPost}
        idPost={params}
        getAllUserCommentPostByPost={getAllUserCommentPostByPost}
      />
    );
  };
  ...
  const getCheckUserLike = () => {
    http
      .get(`api/user_like_post/check_like/${idUser}/${params}`)
      .then((res) => {
  ...
  useEffect(() => {
    if (idUser !== "") getCheckUserLike();
  }, [idUser]);

  const handleLikePost = () => {
    http
      .post(`api/user_like_post`, {
  ...
  const handleUnLikePost = () => {
    http.del(`api/user_like_post/${idUser}/${params}`).then((res) => {
      http
        .del(
          `api/post_notifications/by_action_like/LIKE/${params}/${idUser}/${
            idUserOther === undefined ? idUser : idUserOther
          }`
        )
        .then((res) => {});
  ...
  const getCountUserLike = () => {
    http.get(`api/user_like_post/count_user/${params}`).then((res) => {
      ...
      http
        ...
        .then((res) => {
          http
            ...
            .then((res) => {
              console.log(res);
              setListAllUserCommentPostByPost((prevComments) => [
                res.result,
                ...prevComments,
              ]);
            });
        ...
        <div className={cx("detail_post__engagement")}>
          <div className={cx("detail_post__engagement__header")}>
            <div className={cx("detail_post__engagement__avatar_and_name")}>
              <div
                className={cx("detail_post__engagement__avatar")}
                onClick={() => (window.location.href = `/${idUserOther}`)}
              >
                <img src={avatarUserOther} alt='' />
              </div>
              <div
                className={cx("detail_post__engagement__name")}
                onClick={() => (window.location.href = `/${idUserOther}`)}
              >
            ...
            <div
              className={cx("detail_post__engagement__options")}
              style={{ display: "none" }}
            >
              <svg
          ...
          <div className={cx("detail_post__engagement__list_comment")}>
            <div className={cx("detail_post__engagement__comment_container")}>
              ...
              <div className={cx("detail_post__engagement__content_comment")}>
                <div className={cx("detail_post__engagement__id_and_comment")}>
                  <span
                    className={cx("detail_post__engagement__id")}
                    onClick={() => (window.location.href = `/${idUserOther}`)}
                  >
                    {idUserOther}
                  </span>
                  <span className={cx("detail_post__engagement__comment")}>
                    {caption}
                  </span>
                </div>
                <div
                  className={cx(
                    "detail_post__engagement__time_create_comment_and_option_comment"
                  )}
                >
                  <span
                    className={cx(
                      "detail_post__engagement__time_create_comment"
                    )}
                  >
                    {new Date(dateTimeCreate).toLocaleString()}
                  </span>
                  <div
                    className={cx("detail_post__engagement__option_comment")}
            ...
            {listAllUserCommentPostByPost.map((res) => {
              return (
                <div
                  key={res.commentPost.idCommentPost}
                  className={cx("detail_post__engagement__comment_container")}
                  onMouseEnter={() =>
                    handleMouseEnterComment(res.commentPost.idCommentPost)
                  }
                >
                  <div
                    className={cx("detail_post__engagement__avatar")}
                    onClick={() =>
                      (window.location.href = `/${res.user.idUser}`)
                    }
                  >
                    <img
                      src={
                  ...
                  <div
                    className={cx("detail_post__engagement__content_comment")}
                  >
                    <div
                      className={cx("detail_post__engagement__id_and_comment")}
                    >
                      <span
                        className={cx("detail_post__engagement__id")}
                        onClick={() =>
                    ...
                    >
                      ...
                      >
                        {new Date(
                          res.commentPost.dateTimeComment
                        ).toLocaleString()}
                      </span>
                      <div
                        className={cx(
                          "detail_post__engagement__option_comment"
                        )}
                        style={{
                          visibility:
                            hoveredCommentIndex ===
                              res.commentPost.idCommentPost &&
                      ...
                      >
                        <svg
                          ...
                          width='24'
                        >
                          <title>Tùy chọn bình luận</title>
                          <circle cx='12' cy='12' r='1.5'></circle>
                          <circle cx='6' cy='12' r='1.5'></circle>
                          <circle cx='18' cy='12' r='1.5'></circle>
                        </svg>
          ...
          <div className={cx("detail_post__engagement__button_engagement")}>
            <div
              className={cx("detail_post__engagement__like")}
              style={{ display: isLikePost === false ? "flex" : "none" }}
            ...
            <div
              className={cx("detail_post__engagement__un_like")}
              style={{ display: isLikePost === true ? "flex" : "none" }}
            ...
            <div
              className={cx("detail_post__engagement__comment")}
          ...
          <div
            className={cx("detail_post__engagement__total_like")}
            onClick={handleOpenListUserLikePost}
          >
          ...
          <div className={cx("detail_post__engagement__footer")}>
            ...
            <input
              ...
              ref={inputIputRef}
            />
            <span ref={submitInputRef} onClick={handleSubmitValue}>
              Đăng
            </span>
          </div>
        </div>
