...
function ListUserLikePost({ idPost }) {
    ...
    const getAllUserLikePostByPost = () => {
        http.get(`api/user_like_post/by_post/${idPost}`)
        .then((res) => {
        ...
        <div className={(cx('list_user_like_post__container'))}>
            ...
            <div className={(cx('list_user_like_post__list__container'))}>
                {listUserLikePostByPost.map((res) => {
                    return (
                        <div key={res.user.idUser} className={(cx('list_user_like_post__list__item'))}>
                            <div className={(cx('list_user_like_post__list__item__avatar'))} onClick={() => window.location.href = `/${res.user.idUser}`}>
                                <img src={(res.user.avatar === null || res.user.avatar === '') ? defaultAvatar : res.user.avatar} alt='' />
                            </div>
                            <div className={(cx('list_user_like_post__list__item__id_and_name'))}>
                                <div className={(cx('list_user_like_post__list__item__id'))} onClick={() => window.location.href = `/${res.user.idUser}`}>
